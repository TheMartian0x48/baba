...
{
    arena_dealloc(header->arena, *array);
...
void* array_grow(ArrayHeader* header, void* arr, u64 item_size)
{
    const u64 old_capacity = item_size * header->capacity;
    const u64 new_capacity = item_size * 2 * header->capacity;
    void*     ptr          = arena_realloc(header->arena, arr, old_capacity, new_capacity, header->alignment);
    header->capacity *= 2;
...
void array_add(ArrayHeader* header, void** array, void* element, u64 item_size)
{
    ...
    if (header->capacity < header->size + 1) {
        void* ptr = array_grow(header, *array, item_size);
        *array    = ptr;
    }
...
#define PrimitiveArrayMakeAligned(STRUCT_NAME, TYPE_NAME)                                                                   \
    ...
    result.header.alignment = alignment;                                                                                    \
    result.header.arena     = arena;                                                                                        \
    result.v                = ptr;                                                                                          \
    return result;
...
#define PrimitiveArrayKill                                                                                                  \
    if (array == nullptr)                                                                                                   \
        return;                                                                                                             \
    arena_dealloc(array->header.arena, array->v);                                                                           \
    array->header.arena     = nullptr;                                                                                      \
    array->header.capacity  = 0;                                                                                            \
    array->header.size      = 0;                                                                                            \
    array->header.alignment = 0;                                                                                            \
    array->v                = nullptr;
...
#define PrimitiveArrayAdd(TYPE_NAME)                                                                                        \
    ...
    TYPE_NAME    item_size = sizeof(TYPE_NAME);                                                                             \
                                                                                                                            \
    if (header->capacity < header->size + 1) {                                                                              \
        void* ptr = array_grow(header, array->v, item_size);                                                                \
        array->v  = (TYPE_NAME*)ptr;                                                                                        \
    }                                                                                                                       \
...
#define PrimitiveArrayInsert(TYPE_NAME)                                                                                     \
    assert(element && "element cannot be null");                                                                            \
    assert(array && "array cannot be null");                                                                                \
    if (array->header.size + 1 > array->header.capacity) {                                                                  \
        void* ptr = array_grow(&array->header, array->v, sizeof(TYPE_NAME));                                                \
        array->v  = ptr;                                                                                                    \
    }                                                                                                                       \
    uptr src  = (uptr)array->v + sizeof(TYPE_NAME) * index;                                                                 \
    uptr dest = (uptr)array->v + sizeof(TYPE_NAME) * index + sizeof(TYPE_NAME);                                             \
...
U8Array u8_array_make_aligned(Arena* arena, u64 capacity, u64 alignment) { PrimitiveArrayMakeAligned(U8Array, u8); }
U8Array u8_array_make(Arena* arena, u64 capacity) { return u8_array_make_aligned(arena, capacity, DEFAULT_ALIGNMENT); }
void    u8_array_kill(U8Array* array) { PrimitiveArrayKill; }
void    u8_array_add(U8Array* array, u8 element) { PrimitiveArrayAdd(u8); }
void    u8_array_clear(U8Array* array) { PrimitiveArrayClear; }
void    u8_array_insert(U8Array* array, u8 element, u64 index) { PrimitiveArrayInsert(u8); }
void    u8_array_remove(U8Array* array, u64 index) { PrimitiveArrayRemove(u8); }
...
void     u16_array_kill(U16Array* array) { PrimitiveArrayKill; }
...
void     u32_array_kill(U32Array* array) { PrimitiveArrayKill; }
...
{
    PrimitiveArrayRemove(u32);
    // assert(array && "array cannot be null");
    // if (array->header.size == array->header.capacity) {
    //     array->header.size--;
    //     return;
    // }
    // void* src  = (void*)((uptr)array->v + (index + 1) * sizeof(u32));
    // void* dest = (void*)((uptr)array->v + index * sizeof(u32));
    // u64   size = array->header.size - index - 1;
    // mem_move(dest, src, size * sizeof(u32));
    // array->header.size--;
}
...
void     u64_array_kill(U64Array* array) { PrimitiveArrayKill; }
...
void    i8_array_kill(I8Array* array) { PrimitiveArrayKill; }
...
void     i16_array_kill(I16Array* array) { PrimitiveArrayKill; }
...
void     i32_array_kill(I32Array* array) { PrimitiveArrayKill; }
...
void     i64_array_kill(I64Array* array) { PrimitiveArrayKill; }
...
void    b8_array_kill(B8Array* array) { PrimitiveArrayKill; }
...
void     b16_array_kill(B16Array* array) { PrimitiveArrayKill; }
...
void     b32_array_kill(B32Array* array) { PrimitiveArrayKill; }
...
void     b64_array_kill(B64Array* array) { PrimitiveArrayKill; }
...
void     f32_array_kill(F32Array* array) { PrimitiveArrayKill; }
...
void     f64_array_kill(F64Array* array) { PrimitiveArrayKill; }
